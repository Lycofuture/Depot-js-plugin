 ...
 * @LastEditTime: 2023-07-29 10:27:16
 */
...
export class Setu extends plugin {
  constructor() {
    super({
      ...
      rule: [
        {
          reg: '^#?(涩|色|濏|瑟|铯)(铯|濏|涩|色|瑟|图)(pro|plus)?(.*)$',
          fnc: 'sese'
        }
      ]
    })
  ...
  async sese(e) {
    if (e.msg.includes('pro')) {
      ...
      let ccse = await common.makeForwardMsg(e, msgList, dec, msgsscr)
      let cs = await e.reply(ccse)
      if (!cs) {
      ...
      let ccse = await common.makeForwardMsg(e, data, dec, msgsscr)
      let cs = await e.reply(ccse, false, { recallMsg: 60 })
      if (!cs) {
        e.reply('好、好涩(//// ^ ////)……不、不行啦……被、被吞啦o(≧口≦)o')
      }
      logger.info(cs)
