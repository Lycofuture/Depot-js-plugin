/**
 * @Author: Le-niao
 * @Date: 2023-05-02 17:12:59
 * @LastEditors: Lycofuture 
 * @LastEditTime: 2023-07-29 10:27:40
 */
...
export class Update extends plugin {
  constructor() {
    ...
    this.typeName = true ? 'Yunzai-Bot' : 'Miao-Yunzai'
  }
  async update() {
  ...
  async runUpdate(plugin = '') {
    ...
    if (this.e.msg.includes('强制')) {
      type = '强制更新'
      cm = `git fetch --all && git reset --hard && git pull`
    }
    if (plugin) {
      if (this.e.msg.includes('强制')) {
        type = '强制更新'
  ...
  async getcommitId(plugin = '') {
    ...
    let commitId = await execSync(cm, {
      encoding: 'utf-8'
    })
  ...
  async getLog(plugin = '') {
    ...
    log = await common.makeForwardMsg(this.e, [end, log], end)
    return log
  }
  async updateLog() {
