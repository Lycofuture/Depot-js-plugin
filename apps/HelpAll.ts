 ...
 * @LastEditTime: 2023-07-06 14:13:45
 */
...
export class HelpAll extends plugin {
  ...
  async helpall(e: Messagetype): Promise<boolean> {
    ...
    const results = []
    await traverseDirectories(pluginsDir, filePath => readJavaScriptFile(filePath, results))
    ...
    //   const listns = cleanedResults.map(
    //     (result, index) => `${index + 1}. ${result}\n==============\n`
    //   )
    //   console.info(await e.reply(`==============\n${listns.join('')}`))
    if (cleanedResults.length > 200) {
      await e.reply('指令较多，多张图处理')
      for (let i = 0; i < cleanedResults.length; i += 100) {
        let helpResults = cleanedResults.slice(i, i + 100)
        const image = await helpimage(helpResults, browser)
        await e.reply(image)
      }
    } else {
      console.info('处理图片一张图')
      const image = await helpimage(cleanedResults, browser)
      await e.reply(image)
    }
    console.info('Puppeteer Chromium 已关闭')
    await browser.close()
...
/**
 * @description:
 * @param {any[]} helpResults - 数据组
 * @param {Browser} browser - 浏览器
 * @return {*}
 */
async function helpimage(helpResults: any[], browser): Promise<Buffer> {
  ...
  try {
    ...
    console.info(`[图片生成] 图片渲染成功`)
    return image
  } catch (error) {
    console.error('[图片生成] 图片渲染出现错误！')
    console.error(error)
  }
  await sleep(1000)
}
// 异步函数：读取目录，不包括符号链接
async function readdirNoSymlink(directory: string): Promise<string[]> {
