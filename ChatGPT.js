...
export class ChatGPT extends plugin {
    ...
    async scgpt(e) {
        ...
        if (surl) {
            if (parseInt(data[e.user_id].num) === 0) {
                const txtentso = JSON.parse(surl);
    ...
    async yqkai() {
        ...
        if (/^结束对话/.test(msg)) {
            this.finish('yqkai', false)
            await this.e.reply('已结束对话', true)
        } else {
            let selectedUrl = urls[0];
            let selectedParams = paramsArray(msg, 0);
            ...
            const txtentso = JSON.parse(surl);
            if (this.e.isGroup) {
                await this.e.reply(txtentso.content, true);
            } else {
                await this.e.reply(txtentso.content);
            }
        }
    }
